...
- 官方基线为 `v0.2.5`，当前源码已同步到上游 `v0.2.7` 线。对外版本号以 `backend/cmd/server/VERSION` 为准，当前是 `0.2.7-geili.8`；上线后控制台左上角显示 `v0.2.7-geili.8`。候选构建必须注入该文件，不要再写 `*.acceptance`。下次热修递增为 `0.2.7-geili.9`。
