export default {
  ...
  keys: {
    ...
    routeSelected: '最终选中',
    selectPlan: '请选择订阅套餐',
    planGroupRateHint: '实际消耗取决于所选分组的订阅倍率。',
    groupModelCatalogHint: '复合 Key 只会路由到模型范围明确的分组。请配置模型白名单或账号模型映射；仅选择 OpenAI 协议不会自动支持所有模型。',
    billingSource: "结算方式",
    balanceOnly: "仅使用余额",
    specifiedSubscription: "指定订阅",
    legacySettlement: "保持原结算方式（旧密钥）",
    legacySettlementHint: "修改结算方式后，此密钥将使用新的分组与订阅配置。",
    compositeKey: "复合 Key",
    ...
    planQuotaHint: "额度由套餐统一配置；修改后适用于关联订阅，不会清零已用额度或更改有效期。留空或 0 表示不限额。",
    quotaConsumption: "订阅额度消耗",
    billingSourceColumn: "结算来源",
    ...
    groupLabel: '分组',
    routeDefault: '默认线路',
    subscriptionRequired: '请选择订阅额度。',
    failedToLoadRoutes: '加载订阅线路失败。',
    routePreferencesLabel: '供应商默认线路',
    routePreferencesHint: '为各供应商选择默认线路；倍率决定共享订阅额度的消耗速度。',
