...
export default {
  channelMonitorV2: {
    ...
    settings: {
      ...
      platformsHint: '仅统计填写的模型；未填写的模型不参与统计，留空则不监控该平台。',
      modelsPlaceholder: '填写要统计的完整模型名，以英文逗号分隔；留空不统计',
      badgeNoModels: '未配置模型',
      badgeListedModels: '仅指定模型',
      groupsTitle: '监控分组',
      ...
      namedModelsEmpty: '已启用平台均未配置模型：渠道监控将显示无数据，不统计任何模型。',
      namedModelsCount: '仅统计已启用平台中配置的 {count} 个模型维度；名单外模型不参与任何监控统计。',
      ...
      userContract: {
        ...
        models: '按各平台配置的完整模型名精确统计；名单外模型不统计，模型列表留空则不监控该平台。',
      },
    },
    admin: {
