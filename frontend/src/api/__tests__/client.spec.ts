...
describe('API Client', () => {
  ...
  describe('请求拦截器', () => {
    ...
    it('保留财务查询显式指定的北京时间', async () => {
      const adapter = vi.fn().mockResolvedValue({ status: 200, data: { code: 0, data: {} }, headers: {}, config: {}, statusText: 'OK' })
      apiClient.defaults.adapter = adapter
      await apiClient.get('/usage', { params: { date_basis: 'accounting', timezone: 'Asia/Shanghai' } })
      expect(adapter.mock.calls[0][0].params).toMatchObject({ timezone: 'Asia/Shanghai', date_basis: 'accounting' })
    })
