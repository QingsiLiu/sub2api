...
func isOpenAIAccountCandidateBetter(left openAIAccountCandidateScore, right openAIAccountCandidateScore) bool {
	...
	if left.account.SchedulingPriority() != right.account.SchedulingPriority() {
		return left.account.SchedulingPriority() < right.account.SchedulingPriority()
	}
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	minPriority, maxPriority := filtered[0].SchedulingPriority(), filtered[0].SchedulingPriority()
	maxWaiting := 1
	loadRateSum := 0.0
	...
	for _, account := range filtered {
		...
		if account.SchedulingPriority() < minPriority {
			minPriority = account.SchedulingPriority()
		}
		if account.SchedulingPriority() > maxPriority {
			maxPriority = account.SchedulingPriority()
		}
	...
	for i := range candidates {
		...
		if maxPriority > minPriority {
			priorityFactor = 1 - float64(item.account.SchedulingPriority()-minPriority)/float64(maxPriority-minPriority)
		}
