...
func TestSchedulerSnapshotListSchedulableAccounts_AppliesGroupPriority(t *testing.T) {
	groupID := int64(77)
		...
		snapshot: []*Account{
			{
				ID:            1,
				Priority:      1,
				AccountGroups: []AccountGroup{{AccountID: 1, GroupID: groupID, Priority: 9}},
			},
			{
				ID:            2,
				Priority:      20,
				AccountGroups: []AccountGroup{{AccountID: 2, GroupID: groupID, Priority: 2}},
			},
		},
	}
	...
	require.Equal(t, 9, accounts[0].SchedulingPriority())
	require.Equal(t, 2, accounts[1].SchedulingPriority())
	require.Equal(t, 1, accounts[0].Priority)
	require.Equal(t, 20, accounts[1].Priority)
}

func TestCopyGroupSchedulingPriority_PreservesOnlyTransientValue(t *testing.T) {
	priority := 6
	source := &Account{Priority: 40, GroupPriority: &priority}
	destination := &Account{Priority: 3}

	copyGroupSchedulingPriority(destination, source)

	require.Equal(t, 3, destination.Priority)
	require.Equal(t, 6, destination.SchedulingPriority())
	require.NotSame(t, source.GroupPriority, destination.GroupPriority)
}
