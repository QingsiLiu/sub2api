...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	var routeBillingSnapshot sql.NullString
	if log.RouteBillingSnapshot != nil {
		encoded, err := json.Marshal(log.RouteBillingSnapshot)
		if err == nil {
			routeBillingSnapshot = sql.NullString{String: string(encoded), Valid: true}
		}
	}
