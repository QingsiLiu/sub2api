...
func (r *usageLogRepository) GetUsageTrendWithUsageFilters(ctx context.Context, startTime, endTime time.Time, granularity string, filters UsageLogFilters) (results []TrendDataPoint, err error) {
	return r.getUsageTrendWithFilters(ctx, startTime, endTime, granularity, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.ModelFilterSource, filters.RequestType, filters.Stream, filters.BillingType, filters.BillingMode, filters.UpstreamModelMismatch, filters.NativeCompactionV2, filters.AccountIDs, filters.SubscriptionID)
}

func (r *usageLogRepository) getUsageTrendWithFilters(ctx context.Context, startTime, endTime time.Time, granularity string, userID, apiKeyID, accountID, groupID int64, model string, modelSource string, requestType *int16, stream *bool, billingType *int8, billingMode string, upstreamModelMismatch *bool, nativeCompactionV2 *bool, accountIDs []int64, subscriptionIDs ...int64) (results []TrendDataPoint, err error) {
	if len(accountIDs) == 0 && (len(subscriptionIDs) == 0 || subscriptionIDs[0] <= 0) && shouldUsePreaggregatedTrend(granularity, userID, apiKeyID, accountID, groupID, model, requestType, stream, billingType, billingMode, upstreamModelMismatch, nativeCompactionV2) {
	...
	if len(subscriptionIDs) > 0 && subscriptionIDs[0] > 0 {
		query += fmt.Sprintf(" AND subscription_id = $%d", len(args)+1)
		args = append(args, subscriptionIDs[0])
	}
	if userID > 0 {
	...
	query, args = appendUsageAccountIDsQueryGeili(query, args, accountIDs, "account_id")
	if groupID > 0 {
...
func (r *usageLogRepository) GetModelStatsWithUsageFiltersBySource(ctx context.Context, startTime, endTime time.Time, filters UsageLogFilters, source string) (results []ModelStat, err error) {
	return r.getModelStatsWithFiltersBySource(ctx, startTime, endTime, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.RequestType, filters.Stream, filters.BillingType, source, filters.BillingMode, filters.UpstreamModelMismatch, filters.NativeCompactionV2, filters.AccountIDs, filters.SubscriptionID)
}

func (r *usageLogRepository) getModelStatsWithFiltersBySource(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, source string, billingMode string, upstreamModelMismatch *bool, nativeCompactionV2 *bool, accountIDs []int64, subscriptionIDs ...int64) (results []ModelStat, err error) {
	actualCostExpr := "COALESCE(SUM(actual_cost), 0) as actual_cost"
	// 当仅按 account_id 聚合时，实际费用使用账号倍率（total_cost * account_rate_multiplier）。
	if (accountID > 0 || len(accountIDs) > 0) && userID == 0 && apiKeyID == 0 {
...
func (r *usageLogRepository) getGroupStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, billingMode string, upstreamModelMismatch *bool, nativeCompactionV2 *bool, accountIDs []int64, subscriptionIDs ...int64) (results []usagestats.GroupStat, err error) {
	query := `
		SELECT
	...
	args := []any{startTime, endTime}
	if len(subscriptionIDs) > 0 && subscriptionIDs[0] > 0 {
		query += fmt.Sprintf(" AND ul.subscription_id = $%d", len(args)+1)
		args = append(args, subscriptionIDs[0])
	}
	if userID > 0 {
	...
	query, args = appendUsageAccountIDsQueryGeili(query, args, accountIDs, "ul.account_id")
	if groupID > 0 {
...
func (r *usageLogRepository) GetUserBreakdownStats(ctx context.Context, startTime, endTime time.Time, dim usagestats.UserBreakdownDimension, limit int) (results []usagestats.UserBreakdownItem, err error) {
	...
	query, args = appendUsageAccountIDsQueryGeili(query, args, dim.AccountIDs, "ul.account_id")
