...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		routeBillingSnapshot      sql.NullString
	...
	if routeBillingSnapshot.Valid {
		if err := json.Unmarshal([]byte(routeBillingSnapshot.String), &log.RouteBillingSnapshot); err != nil {
			return nil, fmt.Errorf("decode route billing snapshot: %w", err)
		}
	}
