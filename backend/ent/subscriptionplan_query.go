...
func (_q *SubscriptionPlanQuery) QueryGroupEntitlements() *SubscriptionPlanGroupQuery {
...
func (_q *SubscriptionPlanQuery) WithGroupEntitlements(opts ...func(*SubscriptionPlanGroupQuery)) *SubscriptionPlanQuery {
	query := (&SubscriptionPlanGroupClient{config: _q.config}).Query()
...
func (_q *SubscriptionPlanQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*SubscriptionPlan, error) {
	...
	if query := _q.withGroupEntitlements; query != nil {
		if err := _q.loadGroupEntitlements(ctx, query, nodes,
			func(n *SubscriptionPlan) { n.Edges.GroupEntitlements = []*SubscriptionPlanGroup{} },
...
func (_q *SubscriptionPlanQuery) loadGroupEntitlements(ctx context.Context, query *SubscriptionPlanGroupQuery, nodes []*SubscriptionPlan, init func(*SubscriptionPlan), assign func(*SubscriptionPlan, *SubscriptionPlanGroup)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(subscriptionplangroup.FieldSubscriptionPlanID)
	}
	query.Where(predicate.SubscriptionPlanGroup(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(subscriptionplan.GroupEntitlementsColumn), fks...))
	}))
