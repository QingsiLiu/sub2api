...
func (Group) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Float("rate_multiplier").
			...
			Default(1.0),
		field.Bool("subscription_enabled").Default(false),
		field.Float("subscription_rate_multiplier").
			SchemaType(map[string]string{dialect.Postgres: "decimal(10,4)"}).
			Default(1.0).
		...
		field.JSON("model_plaza_config", domain.GroupModelPlazaConfig{}).
			Default(domain.GroupModelPlazaConfig{Mode: domain.GroupModelPlazaModeAll}).
			SchemaType(map[string]string{dialect.Postgres: "jsonb"}).
			Comment("模型广场展示配置，仅影响展示，不改变模型调用权限"),
