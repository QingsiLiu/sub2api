...
type Group struct {
	...
	// 模型广场展示配置，仅影响展示，不改变模型调用权限
	ModelPlazaConfig domain.GroupModelPlazaConfig `json:"model_plaza_config,omitempty"`
...
type GroupEdges struct {
	...
	SubscriptionEntitlements []*UserSubscriptionGroup `json:"subscription_entitlements,omitempty"`
	// PlanEntitlements holds the value of the plan_entitlements edge.
	PlanEntitlements []*SubscriptionPlanGroup `json:"plan_entitlements,omitempty"`
...
// was not loaded in eager-loading.
func (e GroupEdges) SubscriptionEntitlementsOrErr() ([]*UserSubscriptionGroup, error) {
	if e.loadedTypes[3] {
...
// PlanEntitlementsOrErr returns the PlanEntitlements value or an error if the edge
// was not loaded in eager-loading.
func (e GroupEdges) PlanEntitlementsOrErr() ([]*SubscriptionPlanGroup, error) {
	if e.loadedTypes[4] {
		return e.PlanEntitlements, nil
	}
	return nil, &NotLoadedError{edge: "plan_entitlements"}
}
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldModelPlazaConfig:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field model_plaza_config", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.ModelPlazaConfig); err != nil {
					return fmt.Errorf("unmarshal field model_plaza_config: %w", err)
				}
			}
...
func (_m *Group) QuerySubscriptionEntitlements() *UserSubscriptionGroupQuery {
	return NewGroupClient(_m.config).QuerySubscriptionEntitlements(_m)
}

// QueryPlanEntitlements queries the "plan_entitlements" edge of the Group entity.
func (_m *Group) QueryPlanEntitlements() *SubscriptionPlanGroupQuery {
	return NewGroupClient(_m.config).QueryPlanEntitlements(_m)
}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("model_plaza_config=")
	builder.WriteString(fmt.Sprintf("%v", _m.ModelPlazaConfig))
	builder.WriteString(", ")
