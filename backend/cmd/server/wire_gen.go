...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	billingCacheService := service.ProvideBillingCacheService(billingCache, userRepository, userSubscriptionRepository, apiKeyRepository, userRPMCache, userGroupRateRepository, configConfig, serviceUserPlatformQuotaRepository, usageBillingRepository)
	...
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, apiKeyCache, configConfig, compositeModelRouteRepository, billingCacheService, concurrencyService)
	...
	usageSettlementWorker, err := service.ProvideUsageSettlementWorker(usageBillingRepository, billingCacheService, deferredService, configConfig, openAIGatewayService)
	if err != nil {
		return nil, err
	}
	opsService := service.ProvideOpsService(opsRepository, settingRepository, configConfig, accountRepository, userRepository, concurrencyService, gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, opsSystemLogSink, settingService, authCacheInvalidationWorker, usageSettlementWorker, apiKeyService)
	...
	benefitCampaignService := service.NewBenefitCampaignService(client, subscriptionService)
	benefitCampaignHandler := admin.NewBenefitCampaignHandler(benefitCampaignService)
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db)
	openCodeGoUsageService := service.ProvideOpenCodeGoUsageService(accountRepository, httpUpstream, settingService, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, benefitCampaignHandler, upstreamBillingProbeService, ollamaCloudUsageService, openCodeGoUsageService)
	...
	groupModelCatalog := service.NewGroupModelCatalog(accountRepository, openAIGatewayService)
	modelPlazaService := service.NewModelPlazaService(groupModelCatalog, channelRepository, groupRepository, pricingService, billingService, modelPricingResolver)
	...
	auapiImageTaskRepository := repository.NewAUAPIImageTaskRepository(db)
	auapiImageTaskService := service.ProvideAUAPIImageTaskService(auapiImageTaskRepository, accountRepository, imageStorageSettingService, openAIGatewayService, usageLogRepository, usageBillingRepository, apiKeyAuthCacheInvalidator, billingCacheService, configConfig)
	asyncImageHandler := handler.NewAsyncImageHandlerWithAUAPI(imageTaskService, openAIGatewayHandler, auapiImageTaskService)
