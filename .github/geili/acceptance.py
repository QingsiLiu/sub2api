...
def verify():
    ...
    gid=groups['stable']['id']
    api('/admin/groups/'+str(gid),{'peak_rate_enabled':True,'peak_start':'00:00','peak_end':'23:59','peak_rate_multiplier':2},admin,'PUT')
    row=call('stable');check('subscription applies peak exactly once',abs(row['actual_cost']-.0024)<1e-9,row)
    call('stable',credential=balanceKey['key']);row=lastlog(balanceKey['id'])
    check('balance applies peak to balance rate',abs(row['actual_cost']-.00048)<1e-9,row)
    api('/admin/groups/'+str(gid),{'peak_rate_enabled':False},admin,'PUT')
    api('/admin/groups/'+str(gid)+'/rate-multipliers',{'entries':[{'user_id':uid,'rate_multiplier':.7}]},admin,'PUT')
    row=call('stable');check('user override replaces subscription base',abs(row['actual_cost']-.00084)<1e-9,row)
    call('stable',credential=balanceKey['key']);row=lastlog(balanceKey['id'])
    check('user override replaces balance base',abs(row['actual_cost']-.00084)<1e-9,row)
    api('/admin/groups/'+str(gid)+'/rate-multipliers',token=admin,method='DELETE')
