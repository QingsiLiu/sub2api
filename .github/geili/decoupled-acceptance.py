...
PRIVATE=ROOT/'deploy/.secrets/decoupled-acceptance'
BASE='http://127.0.0.1:18492'
PG='geili-acceptance-local-postgres'
DB='acceptance_decoupled'
...
def admin():
 login=json.loads(Path('/Users/tedliu/code/geili/sub2api/deploy/.secrets/subscription-lab-login.json').read_text())
...
def serve():
 ...
 login=json.loads(Path('/Users/tedliu/code/geili/sub2api/deploy/.secrets/subscription-lab-login.json').read_text())
 # Seed meaningful old records before the new migrations. Idempotent on restart.
 sql("""
 ...
 INSERT INTO api_keys(user_id,group_id,name,key,status)
 SELECT u.id,g.id,'Legacy unchanged','sk-decoupling-legacy-fixture','active' FROM users u CROSS JOIN groups g
 WHERE u.email='decoupling-legacy@example.invalid' AND g.name='decoupling-legacy-fixture'
 ...
 mock=base.http.server.ThreadingHTTPServer(('127.0.0.1',18992),DecoupledMock)
 threading.Thread(target=mock.serve_forever,daemon=True).start()
 runtime=PRIVATE/'runtime';runtime.mkdir(exist_ok=True,mode=0o700)
 env={**os.environ,'AUTO_SETUP':'true','SERVER_HOST':'127.0.0.1','SERVER_PORT':'18492','DATA_DIR':str(runtime),'DATABASE_HOST':'127.0.0.1','DATABASE_PORT':'15439','DATABASE_USER':'acceptance','DATABASE_DBNAME':DB,'DATABASE_PASSWORD':cfg['password'],'DATABASE_SSLMODE':'disable','REDIS_HOST':'127.0.0.1','REDIS_PORT':'16389','REDIS_DB':'5','ADMIN_EMAIL':'admin@subscription-lab.invalid','ADMIN_PASSWORD':login['password'],'JWT_SECRET':cfg['password']*2,'TOTP_ENCRYPTION_KEY':secrets.token_hex(32),'TZ':'Asia/Shanghai'}
...
def bootstrap():
 ...
 for name,model,rate in [('gpt','gpt-4.1-mini',1.1),('pro','gpt-4.1-mini',1.3),('cn','neutral-custom-national',.8),('down','gpt-4.1-mini',4),('disabled','gpt-4.1-mini',1)]:
  ...
  account=api('/admin/accounts',{'name':'decoupled-'+name+'-'+stamp,'platform':'openai','type':'apikey','credentials':{'api_key':'synthetic-test-key','base_url':'http://127.0.0.1:18992/'+name,'model_mapping':{model:model}},'group_ids':[g['id']],'concurrency':5,'priority':1},token)
...
def verify_edges(s,check,call,snap):
 ...
 def mockcalls():return base.request('/__calls',base='http://127.0.0.1:18992')[1]
 ...
 check('existing subscription reads new plan quota',any(x['id']==sid and x['plan']['daily_limit_usd']==.5 for x in detail))
 api('/admin/payment/plans/'+str(pid),{'daily_limit_usd':.00001},token,'PUT');before=snap();n=count()
 ...
 api('/admin/payment/plans/'+str(pid),{'daily_limit_usd':1},token,'PUT')
 ...
 sql(f"UPDATE user_subscriptions SET expires_at=NOW()-INTERVAL '1 day',status='expired' WHERE id={sid};")
 ...
 api('/admin/accounts',{'name':'unpriced fixture '+str(time.time_ns()),'platform':'openai','type':'apikey','credentials':{'api_key':'synthetic-test-key','base_url':'http://127.0.0.1:18992/unpriced','model_mapping':{unpriced:unpriced}},'group_ids':[g['id']],'concurrency':5,'priority':1},token)
 ...
 for name in ['bad400','partial']:
  ...
  api('/admin/accounts',{'name':name+' fixture '+str(time.time_ns()),'platform':'openai','type':'apikey','credentials':{'api_key':'synthetic-test-key','base_url':'http://127.0.0.1:18992/'+name,'model_mapping':{model:model}},'group_ids':[g['id']],'concurrency':5,'priority':1},token)
 ...
 # Re-executing the additive migration must not mutate user quota/configuration.
 before=sql("SELECT json_build_object('subs',(SELECT json_agg(row_to_json(s)) FROM user_subscriptions s),'plans',(SELECT json_agg(row_to_json(p)) FROM subscription_plans p),'keys',(SELECT json_agg(json_build_object('id',id,'billing_source',billing_source,'group_id',group_id,'group_ids',group_ids)) FROM api_keys));")[0]
 sql((ROOT/'backend/migrations/248_decouple_subscription_plans_and_keys.sql').read_text())
 after=sql("SELECT json_build_object('subs',(SELECT json_agg(row_to_json(s)) FROM user_subscriptions s),'plans',(SELECT json_agg(row_to_json(p)) FROM subscription_plans p),'keys',(SELECT json_agg(json_build_object('id',id,'billing_source',billing_source,'group_id',group_id,'group_ids',group_ids)) FROM api_keys));")[0]
 check('migration rerun does not duplicate or reset records',before==after)
 ...
 check('legacy subscription identity term and usage preserved',legacy==json.loads((PRIVATE/'pre-migration.json').read_text()))
